...
@section('content')
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
        <div>
            <h3 class="fw-bold mb-3">Data Kelengkapan Pendaftar</h3>
            <h6 class="op-7 mb-2">Kelola data kelengkapan pendaftar</h6>
        </div>
    </div>
            ...
            <div class="card card-round">
                <div class="card-header">
                    <div class="card-head-row card-tools-still-right">
                        <div class="card-title">Daftar Kelengkapan Pendaftar</div>
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-5">
                            <form method="GET" action="{{ route('kelengkapan-pendaftar.index') }}" id="filterForm">
                        ...
                        <div class="col-md-5">
                            <form method="GET" action="{{ route('kelengkapan-pendaftar.index') }}" id="searchForm">
                                ...
                                <div class="input-group">
                                    ...
                                    @if(request('search') || request('periode_id'))
                                        <a href="{{ route('kelengkapan-pendaftar.index') }}" class="btn btn-secondary">
                                ...
                                <tbody>
                                    @foreach ($pendaftars as $index => $pendaftar)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $kelengkapan->nama_lengkap ?? $pendaftar->nama_lengkap }}</td>
                                            <td>{{ $kelengkapan->email ?? $pendaftar->email }}</td>
                                            <td>
                                                ...
                                                @if(isset($pendaftar->kelengkapan_dokumen))
                                                    <div class="d-flex flex-column">
                                                        @php
                                                            ...
                                                            $statusKelengkapan = $pendaftar->kelengkapan_dokumen['status_kelengkapan'];
                                                        @endphp
                                                        ...
                                                        @if($isAllUploaded)
                                                            <small class="text-success fw-bold">
                                                                <i class="fa fa-check-circle"></i> Dokumen lengkap
                                                            </small>
                                                        @else
                                            ...
                                            <td>
                                                @if ($kelengkapan)
                                                    @if ($kelengkapan->status === 'submitted')
                                                        <span class="badge badge-success">Menunggu Persetujuan</span>
                                                    @elseif($kelengkapan->status === 'verified')
                                                        <span class="badge badge-warning">Berkas Lengkap</span>
                                                    @elseif($kelengkapan->status === 'rejected')
                                                        <span class="badge badge-danger">Berkas Tidak Lengkap</span>
                                                    @else
                                                        <span class="badge badge-secondary">Belum Diverifikasi</span>
                                                    @endif
                                                @else
                                                    <span class="badge badge-secondary">Belum Mengumpulkan</span>
                                                @endif
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="form-button-action">
                                                     <a href="{{ route('kelengkapan-pendaftar.show', ['id' => $pendaftar->id, 'from' => 'menunggu']) }}"
                                                       class="btn btn-link btn-primary btn-lg"
                                                       data-bs-toggle="tooltip"
                                                       title="Lihat Detail">
                                                        <i class="fa fa-eye"></i>
                                                    </a>
                                                    @if(isset($pendaftar->kelengkapan_dokumen['kelengkapan_id']))
                                                        <a href="{{ route('kelengkapan-pendaftar.download', $pendaftar->kelengkapan_dokumen['kelengkapan_id']) }}"
                                                        ...
                                                        <button type="button"
                                                                ...
                                                                data-id="{{ $pendaftar->kelengkapan_dokumen['kelengkapan_id'] }}"
                                                                data-status="{{ $pendaftar->kelengkapan_dokumen['status'] }}"
                                                                data-catatan="{{ $pendaftar->kelengkapan_dokumen['catatan_admin'] ?? '' }}"
                                                                data-nama="{{ $pendaftar->nama_lengkap }}"
                                                                data-bs-toggle="tooltip"
                                                                title="Update Status">
    ...
    <!-- Modal Update Status Kelengkapan -->
            ...
            <div class="modal-content">
                <form id="formUpdateStatus" method="POST" action="">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalUpdateStatusLabel">Update Status Kelengkapan Dokumen</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <input type="hidden" id="kelengkapan_id" name="kelengkapan_id">
                        ...
                        <div class="mb-3">
                            <label for="status_kelengkapan" class="form-label">Status Kelengkapan</label>
                            <select class="form-select" id="status_kelengkapan" name="status" required>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="catatan_admin" name="catatan_admin" class="form-control" rows="3" placeholder="Tambahkan catatan jika diperlukan..."></textarea>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn-warning[data-id]', function(e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                var catatan = button.attr('data-catatan');
                ...
                // Set data ke modal
                document.getElementById('kelengkapan_id').value = id;
                document.getElementById('nama_pendaftar').value = nama;
                document.getElementById('status_kelengkapan').value = status || 'draft';
                document.getElementById('catatan_admin').value = catatan || '';
                
                // Set action form
                var routeTemplate = "{{ route('kelengkapan-pendaftar.update-status', ['id' => ':id']) }}";
                var actionUrl = routeTemplate.replace(':id', id);
                document.getElementById('formUpdateStatus').action = actionUrl;
                
                // Show modal
                var modalEl = document.getElementById('modalUpdateStatus');
