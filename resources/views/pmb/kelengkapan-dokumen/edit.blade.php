...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
        <div class="flex items-start">
            ...
            <div class="ml-3">
                ...
                <div class="mt-3">
                    <a href="{{ route('pmb.dokumen.show', $dokumen) }}" class="text-blue-600 hover:text-blue-500 text-sm font-medium">
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <form action="{{ route('pmb.dokumen.update', $dokumen) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        Kosongkan jika tidak ingin mengubah file. Format yang didukung: JPG, PNG, PDF, DOC, DOCX. Maksimal 5MB.
                    </p>
                </div>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        ...
                        >{{ old('catatan', $dokumen->catatan) }}</textarea>
                    </div>
                    @error('catatan')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                @if($isImage)
                    <div>
                        ...
                        <div class="mt-2 text-center">
                            <img src="{{ $fileUrl }}" alt="Preview dokumen" class="mx-auto max-w-xs h-auto rounded-lg shadow-sm">
                        </div>
                    </div>
                @endif

                <!-- Form Actions -->
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-4 border-t border-gray-200 dark:border-gray-700">
                    <div class="flex items-center space-x-3">
                        ...
                        <a href="{{ route('pmb.dokumen.show', $dokumen) }}" class="inline-flex justify-center py-2 px-4 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            Batal
                        </a>
                    </div>
                    ...
                    <div class="mt-3 sm:mt-0">
                        <form action="{{ route('pmb.dokumen.destroy', $dokumen) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="return confirm('Yakin ingin menghapus dokumen ini? Tindakan ini tidak dapat dibatalkan.')" class="inline-flex justify-center py-2 px-4 border border-red-300 shadow-sm text-sm font-medium rounded-md text-red-700 bg-white hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                                <svg class="-ml-1 mr-2 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
